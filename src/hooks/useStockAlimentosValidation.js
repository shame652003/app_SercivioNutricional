...
export default function useStockAlimentosValidation(navigation) {
  ...
  const generarPdf = async () => {
    try {
      ...
      // Leer imágenes usando la API legacy estable
      ...
      const html = `
        ...
        <head>
          <meta charset="utf-8" />
          <style>
            @page { margin: 40px 30px 60px 30px; @bottom-center { content: "Página " counter(page) " / Servicio Nutricional"; font-size: 12px; color: #666; } }
            body { font-family: 'Helvetica', Arial, sans-serif; margin: 0; padding: 0 24px; color: #333; line-height: 1.4; }
            .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid #0066CC; padding-bottom: 15px; margin-bottom: 30px; }
            .header img { max-height: 70px; width: auto; }
            .header .title-container { flex: 1; text-align: center; color: #0066CC; font-weight: bold; letter-spacing: 1.2px; }
            .header .title-container h1 { margin: 0; font-size: 24px; text-transform: uppercase; letter-spacing: 2px; }
            .header .title-container p { margin: 4px 0 0 0; font-weight: 600; font-size: 14px; color: #222; }
            .report-title { text-align: center; font-size: 20px; color: #0066CC; font-weight: bold; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 2px; }
            .report-summary { text-align: center; font-size: 13px; color: #444; margin-bottom: 25px; font-weight: 500; }
            table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
            th, td { border: 1px solid #3399FF; padding: 8px 6px; text-align: center; font-size: 14px; }
            th { background-color: #0066CC; color: white; }
            tbody tr:nth-child(even) { background-color: #f6faff; }
            tfoot td { font-weight: bold; text-align: right; padding-right: 10px; background-color: #dceeff; }
            tfoot td.total { text-align: center; }
          </style>
        </head>
        <body>
          <div class="header">
  ...
  useEffect(() => {
    ...
    if (searchText.trim() !== '') {
      timeoutRef.current = setTimeout(() => buscarAlimentos(searchText), 700);
    } else {
      setAlimentosFiltrados([]);
      setBusquedaExitosa(false);
      setLoading(false);
